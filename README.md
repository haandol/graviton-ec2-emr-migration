...
5. [Phase 2: Zeppelin 설정 마이그레이션](#5-phase-2-zeppelin-설정-마이그레이션)
6. [Phase 2.5: S3 이관 허브 구성](#6-phase-25-s3-이관-허브-구성)
7. [Phase 3: Graviton 인스턴스 전환 지원](#7-phase-3-graviton-인스턴스-전환-지원)
8. [Phase 4: IaC 템플릿 생성 및 클러스터 프로비저닝](#8-phase-4-iac-템플릿-생성-및-클러스터-프로비저닝)
9. [Phase 4.5: 새 클러스터 데이터/설정 복원](#9-phase-45-새-클러스터-데이터설정-복원)
10. [Phase 4.7: 네트워크 및 데이터 소스 연결 검증](#10-phase-47-네트워크-및-데이터-소스-연결-검증)
11. [Phase 5~6: 테스트·전환 체크리스트 관리](#11-phase-56-테스트전환-체크리스트-관리)
...
### 2.2 EBS 스냅샷 생성 (Optional)

> AMI 생성 시 연결된 EBS 볼륨의 스냅샷이 자동으로 포함되므로, 대부분의 경우 별도 스냅샷은 불필요하다. 다음의 경우에만 별도로 생성한다:
> - AMI 블록 디바이스 매핑에 포함되지 않은 **추가 데이터 볼륨**이 별도로 부착되어 있는 경우
...
Phase 1~2의 코드 변환 및 테스트를 운영 중인 환경에서 직접 수행하면 서비스에 영향을 준다. 스테이징 환경을 별도로 구성하여 안전하게 작업한다.
...
> **주의**: Phase 1~2는 반드시 스테이징 환경에서 수행한다. 운영 환경에 직접 변경을 가하지 않는다.
...
- 실패하는 셀이 있으면 원인과 수정 방안을 제시해줘
```

---

## 5. Phase 2: Zeppelin 설정 마이그레이션
...
## 6. Phase 2.5: S3 이관 허브 구성

스테이징에서 코드 변환(Phase 1~2)이 완료되면, 모든 결과물을 S3에 체계적으로 업로드한다. 이 S3 이관 허브가 기존 환경과 새 Graviton 환경을 잇는 중간 저장소 역할을 한다.
...
### 7.4 단독 EC2 운영 시 부트스트랩 (EMR이 아닌 경우)
...
## 8. Phase 4: IaC 템플릿 생성 및 클러스터 프로비저닝
...
- variables.tf: 인스턴스 타입(m8g.xlarge), AMI(Amazon Linux 2023 ARM64),
  VPC/Subnet/보안그룹, 키페어 등을 변수로
- main.tf: aws_instance 리소스 + user_data로 부트스트랩 스크립트 실행
- bootstrap-graviton-standalone.sh: Spark, Zeppelin, Python 3 설치
- outputs.tf: 인스턴스 ID, 퍼블릭 IP, SSH 접속 명령어
인벤토리 조사 결과의 VPC/Subnet/보안그룹 값을 terraform.tfvars
예시로 작성해줘.
```

### 8.3 기존 Terraform 코드 마이그레이션
...
## 9. Phase 4.5: 새 클러스터 데이터/설정 복원

IaC로 새 Graviton 클러스터를 프로비저닝한 후, S3 이관 허브에서 노트북, 설정, 스크립트를 복원한다.

> **경로 차이 주의**: EMR 관리형 클러스터와 단독 EC2는 Zeppelin 설치 경로가 다르다.
> - **EMR 관리형**: `/usr/lib/zeppelin`, `/usr/lib/spark`
> - **단독 EC2**: `/opt/zeppelin`, `/opt/spark` (설치 방식에 따라 다를 수 있음)
>
> 복원 스크립트의 대상 경로를 환경에 맞게 확인하고 지정해야 한다.
...
## 10. Phase 4.7: 네트워크 및 데이터 소스 연결 검증
...
│       ↓          ※ 스테이징에서 수행                                  │
│       ↓                                                              │
│  Phase 2    ──→  Zeppelin 설정 파일 변환                              │
│       ↓          인터프리터 Python 3 경로 설정                        │
│       ↓                                                              │
│  Phase 2.5  ──→  S3 이관 허브에 변환 결과물 업로드                    │
│       ↓          HDFS 데이터, EBS 데이터, Metastore 이관              │
│       ↓                                                              │
│  Phase 3    ──→  인스턴스 매핑 검증 + ARM64 패키지 호환성 점검       │
│       ↓          부트스트랩 스크립트 커스터마이징                      │
│       ↓                                                              │
│  Phase 4    ──→  Terraform 템플릿 생성                                │
│       ↓          새 Graviton EMR 클러스터 프로비저닝                   │
│       ↓                                                              │
│  Phase 4.5  ──→  S3에서 노트북/설정 복원 + interpreter.json 머지     │
│       ↓          커스텀 스크립트/cron job 복원                         │
│       ↓                                                              │
│  Phase 4.7  ──→  네트워크 연결 검증 (S3, RDS, YARN 등)               │
│       ↓          보안그룹 비교 + IAM 권한 감사                        │
│       ↓                                                              │
│  Phase 5    ──→  자동 검증 스크립트 + Go/No-Go 판정 보조             │
│       ↓                                                              │
│  Phase 6    ──→  Blue-Green 전환 + 트러블슈팅 + 롤백 지원            │
│                  기존 클러스터 7일 유지 후 종료                        │
└──────────────────────────────────────────────────────────────────────┘
```

**핵심 원칙**: Kiro는 자동 변환과 스크립트 생성을 담당하지만, **최종 검토와 실행 판단은 반드시 사람이 수행한다**. 특히 Python 2→3 나눗셈 변경, interpreter.json 머지 검토, 네트워크 연결 확인, 프로덕션 전환 Go/No-Go 판정은 수동 확인이 필수이다.
